...
#define PyCOND_T _cond_var

Py_LOCAL_INLINE(int)
PyCOND_INIT(PyCOND_T *cond) {
    condvarInit(&((cond)->cv), &((cond)->m));
    return 0;
}

#define PyCOND_FINI(cond)       0
#define PyCOND_SIGNAL(cond)     condvarWakeOne(&((cond)->cv))
#define PyCOND_BROADCAST(cond)  condvarWakeAll(&((cond)->cv))
#define PyCOND_WAIT(cond, mut)  condvarWait(&((cond)->cv))
...
PyCOND_TIMEDWAIT(PyCOND_T *cond, PyMUTEX_T *mut, PY_LONG_LONG us)
{
    ...
    r = condvarWaitTimeout(&((cond)->cv), ns);
    if (r == 0xEA01)
        return 1;
    else if (r)
        return -1;
